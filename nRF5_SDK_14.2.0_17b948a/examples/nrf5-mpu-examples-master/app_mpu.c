...
uint32_t app_mpu_config(app_mpu_config_t * config)
{
    ...
    return nrf_drv_mpu_write_registers(MPU_REG_SMPLRT_DIV, data, 4);
		//return nrf_drv_mpu_write_registers(ACCEL_SMPLRT_DIV_2, data, 4); ACCEL_CONFIG, GYRO_CONFIG_1
...
uint32_t app_mpu_read_accel(accel_values_t * accel_values)
{
    ...
    err_code = nrf_drv_mpu_read_registers(MPU_REG_ACCEL_XOUT_H, raw_values, 6);
		//err_code = nrf_drv_mpu_read_registers(ACCEL_XOUT_H, raw_values, 6);
    if(err_code != NRF_SUCCESS) return err_code;
...
uint32_t app_mpu_read_gyro(gyro_values_t * gyro_values)
{
    ...
    err_code = nrf_drv_mpu_read_registers(MPU_REG_GYRO_XOUT_H, raw_values, 6);
		//err_code = nrf_drv_mpu_read_registers(GYRO_XOUT_H, raw_values, 6);
    if(err_code != NRF_SUCCESS) return err_code;
...
{	
	...
	//err_code = nrf_drv_mpu_read_registers(INT_PIN_CFG, (uint8_t *)&bypass_config, 1);
    ...
    return nrf_drv_mpu_write_magnetometer_register(MPU_AK89XX_REG_CNTL, *data);
		//return nrf_drv_mpu_write_magnetometer_register(MAG_CNTL2, *data); write at AK09916_CNTL is not the same, 5 bits for the mode, theres isn't resolution bit 
}
...
uint32_t app_mpu_read_magnetometer(magn_values_t * p_magnetometer_values, app_mpu_magn_read_status_t * p_read_status)
{
	...
	//err_code = nrf_drv_mpu_read_magnetometer_registers(AK09916_XOUT_L, (uint8_t *)p_magnetometer_values, 6);
	if(err_code != NRF_SUCCESS) return err_code;
	...
	if(p_read_status == NULL)
	{
		...
		//err_code = nrf_drv_mpu_read_magnetometer_registers(AK09916_ST2, &status_2_reg, 1);
	}
	else
	{
		...
		//err_code = nrf_drv_mpu_read_magnetometer_registers(AK09916_ST2, (uint8_t *)p_read_status, 1);
	}
	return err_code;
}

uint32_t app_mpu_read_magnbis(magn_values_t * p_magnetometer_values, app_mpu_magn_read_status_t * p_read_status)
{
	uint32_t err_code;
	err_code = nrf_drv_mpu_read_magnetometer_registers(MPU_AK89XX_REG_ASAX, (uint8_t *)p_magnetometer_values, 6);
	if(err_code != NRF_SUCCESS) return err_code;
