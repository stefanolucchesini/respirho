...
{	
	...
	app_icm_reg_bank_select(USER_BANK_0);
	err_code = nrf_drv_mpu_read_registers(UB0_INT_PIN_CFG, (uint8_t *)&prova, 1);  //funziona
	
	// Write magnetometer config data	
	uint8_t *data;
  data = (uint8_t*)p_magnetometer_conf;	
  err_code = nrf_drv_mpu_write_magnetometer_register(MAG_CNTL2, *data); //write to MAG_CNTL2 register to set the operation mode (bit [4:0])
					 //IL PROBLEMA è QUI^^^^^^^^^^^^^^^^^^^^ il magnetometro non risponde
	return err_code;
}       
